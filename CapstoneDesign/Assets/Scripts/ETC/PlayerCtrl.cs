...
public class PlayerCtrl : MonoBehaviour
{
    ...
    public bool[] isOpened = new bool[40]; //오브젝트의 문이 열리거나 동작중인지 파악
    public bool[] crackChk = new bool[15]; //하자 부분 체크
    ...
    void Update()
    {
        ...
        cursorGauge.fillAmount = GaugeTimer; //커서게이지 이미지 채워서 게이지 로드

        if (PlayerPrefs.GetInt("SceneNum") == 2) //방 1-5번이면
        {
            timer += Time.deltaTime; //타이머 시작
            timerGauge.fillAmount = timer / 300.0f; //5분으로 타이머 설정
            time_last = 301.0f - timer; //타이머 제한 시간 설정 (초 - 타이머)
            timertext.text = "남\n" + "은\n" + "시\n" + "간\n" + ((int)(time_last / 60) + "\n" + "분\n" + (int)(time_last % 60) + "\n" + "초"); //남은 시간 표시
            if (time_last <= 60.0f) //남은 시간이 1분 이하일 때
            {
                timertext.text = "남\n" + "은\n" + "시\n" + "간\n" + ((int)(time_last / 60) + "\n" + "분\n" + (int)(time_last % 60) + "\n" + "초"); //남은 시간 표시
            }
            ...
            else if(curSceneName == "Room 1") //1번 방일 때
            {
                ...
                {
                    crackText.text = "남은 하자: 0개"; //남은 하자를 0개로 세팅
            ...
            else if(curSceneName == "Room 2") //2번 방일 때
            {
                ...
                if ((PlayerPrefs.GetInt("crack1") == 1) && (PlayerPrefs.GetInt("crack2") == 1) && (PlayerPrefs.GetInt("crack3") == 1) && (time_last >= 0.0f)) //하자 세 개를 모두 찾았을 때
                {
                    PlayerPrefs.SetInt("pass", 1); //패스값 설정
                }
            }
            else if (curSceneName == "Room 3") //3번 방일 때
            {
                ...
                if ((PlayerPrefs.GetInt("crack4") == 1) && (PlayerPrefs.GetInt("crack5") == 1) && (PlayerPrefs.GetInt("crack6") == 1) && (time_last >= 0.0f)) //하자 세 개를 모두 찾았을 때
                {
                    PlayerPrefs.SetInt("pass", 1); //패스값 설정
                }
            }
            else if(curSceneName == "Room 4") //4번 방일 때
            {
                ...
                if ((PlayerPrefs.GetInt("crack7") == 1) && (PlayerPrefs.GetInt("crack8") == 1) && (PlayerPrefs.GetInt("crack4") == 1) && (PlayerPrefs.GetInt("crack9") == 1) && (time_last >= 0.0f)) //하자 네 개를 모두 찾았을 때
                {
                    PlayerPrefs.SetInt("pass", 1); //패스값 설정
                }
            }
            else if (curSceneName == "Room 5") //5번 방일 때
            {
                ...
                if ((PlayerPrefs.GetInt("crack9") == 1) && (PlayerPrefs.GetInt("crack10") == 1) && (PlayerPrefs.GetInt("crack4") == 1) && (PlayerPrefs.GetInt("crack11") == 1) && (PlayerPrefs.GetInt("crack12") == 1) && (time_last >= 0.0f)) //하자 다섯 개를 모두 찾았을 떄
                {
                    PlayerPrefs.SetInt("pass", 1); //패스값 설정
                }
            }
        ...
        {
            if(hit.transform.tag == "StartBT") //시작 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("StartBT").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "WayBT") //조작법 & 진행방식 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("WayBT").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "ExitBT") //종료 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("ExitBT").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "MainBT") //메인메뉴 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("MainBT").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "SelectBT1") //방 선택화면 버튼 1번이면
            {
                anim = GameObject.FindGameObjectWithTag("SelectBT1").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "SelectBT2") //방 선택화면 버튼 2번이면
            {
                anim = GameObject.FindGameObjectWithTag("SelectBT2").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "SelectBT3") //방 선택화면 버튼 3번이면
            {
                anim = GameObject.FindGameObjectWithTag("SelectBT3").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "SelectBT4") //방 선택화면 버튼 4번이면
            {
                anim = GameObject.FindGameObjectWithTag("SelectBT4").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "SelectBT5") //방 선택화면 버튼 5번이면
            {
                anim = GameObject.FindGameObjectWithTag("SelectBT5").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "Quiz") //계약사항킈즈 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("Quiz").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "RoomSelect") //방 선택화면 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("RoomSelect").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "QuizBT1") //퀴즈 버튼 1번이면
            {
                anim = GameObject.FindGameObjectWithTag("QuizBT1").GetComponent<Animation>(); //애니메이션 설정
                anim.Play();
            }
            else if (hit.transform.tag == "QuizBT2") //퀴즈 버튼 2번이면
            {
                anim = GameObject.FindGameObjectWithTag("QuizBT2").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "QuizBT3") //퀴즈 버튼 3번이면
            {
                anim = GameObject.FindGameObjectWithTag("QuizBT3").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "QuizBT4") //퀴즈 버튼 4번이면
            {
                anim = GameObject.FindGameObjectWithTag("QuizBT4").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "YesBT") //네 버튼 번이면
            {
                anim = GameObject.FindGameObjectWithTag("YesBT").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "NoBT") //아니오 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("NoBT").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }
            else if (hit.transform.tag == "Button") //실제방 버튼이면
            {
                anim = GameObject.FindGameObjectWithTag("Button").GetComponent<Animation>(); //애니메이션 설정
                anim.Play(); //애니메이션 실행
            }

            GaugeTimer += 1.0f / gazeTimer * Time.deltaTime; //게이지 차는 시간을 3초로 세팅
                
            if (GaugeTimer >= 1.0f) //게이지가 다 찼을 때
            {
                if (PlayerPrefs.GetInt("SceneNum") == 0) //씬 넘버가 0이면
                {
                ...
                else if (PlayerPrefs.GetInt("SceneNum") == 2) //방 1-5번 씬일때
                {
                    ...
                    else if ((hit.transform.tag == "YesBT") || (hit.transform.tag == "NoBT")) //네 버튼 또는 아니오 버튼과 상호작용시
                        hit.transform.GetComponent<Button>().onClick.Invoke(); //버튼 이벤트 실행
