    ...
    void Update()
    {
        ...
        if (Physics.Raycast(this.transform.position, forward, out hit)) //바라봤을 때
        {
            ...
            if (GaugeTimer >= 1.0f) //게이지가 다 차면
            {
                ...
                {
                    if (hit.transform.tag == "Untagged") //태그 미지정된 물체이면
                        GaugeTimer = 0.0f; //게이지를 채우지 않음
                    else if (hit.transform.tag == "Crack") //방에서 하자 부분을 발견했을때 (수정할 부분)
                        cnt++;
                    else if (hit.transform.tag == "Manager") //부동산 중개업자 오브젝트를 바라볼 때
                        managerPanel.SetActive(true); //패널 활성화
                    else if (hit.transform.tag == "Button") //패널에 붙어있는 버튼과 상호작용시
                        hit.transform.GetComponent<Button>().onClick.Invoke(); //버튼 이벤트 실행
                    else if ((hit.transform.tag == "RoomDoor" == true) && (isOpened[0] == false)) //방문이 닫혀 있을 때
                    {
                        isOpened[0] = true;
                        audioSource = GameObject.FindGameObjectWithTag("RoomDoor").GetComponent<AudioSource>();
                        audioSource.PlayOneShot(audioClips[0]);
                    }
                    else if ((hit.transform.tag == "RoomDoor" == true) && (isOpened[0] == true)) //방문이 열려 있을 때
                    {
                        isOpened[0] = false;
                        audioSource = GameObject.FindGameObjectWithTag("RoomDoor").GetComponent<AudioSource>();
                    ...
                    else if ((hit.transform.tag == "MicrowaveOpener" == true) && (isOpened[3] == false) && (isOpened[42] == false))
                    {
                        isOpened[3] = true;
                        audioSource = GameObject.FindGameObjectWithTag("MicrowaveOpener").GetComponent<AudioSource>();
                        audioSource.PlayOneShot(audioClips[4]);

                        //동작소리랑 문 여는 소리 작업
                    }
                    else if ((hit.transform.tag == "MicrowaveDoor" == true) && (isOpened[3] == true) && (isOpened[42] == false)) 
                    {
                        isOpened[3] = false;
                    ...
                    else if ((hit.transform.tag == "InductionDoor" == true) && (isOpened[4] == false)) 
                    {
                        isOpened[4] = true;
                    }
                    else if ((hit.transform.tag == "InductionDoor" == true) && (isOpened[4] == true)) 
                    {
                        isOpened[4] = false;
                    }
                    ...
                    else if ((hit.transform.tag == "AC" == true) && (isOpened[41] == true)) 
                    {
                        isOpened[41] = false;
                    }

                    else if ((hit.transform.tag == "MicrowaveOpr" == true) && (isOpened[42] == false) && (isOpened[3] == false)) 
                    {
                        isOpened[42] = true;
                    }
                    else if ((hit.transform.tag == "MicrowaveOpr" == true) && (isOpened[42] == true) && (isOpened[3] == false)) 
                    {
                        isOpened[42] = false;
                    }
                }
        ...
        if (PlayerPrefs.GetInt("SceneNum") == 2) //방 1-5번 씬이면
        {
            if (Input.GetMouseButton(0)) //자석버튼 누르고 있으면
            {
